...
const age = +prompt('Введите свой ворзраст, чтоб получить персональную скидку')
...
console.log((username === 'admin' || username === 'user') && password === '123456' ? 'Доступ разрешен' : 'Доступ запрещен');
...
//        "Экспресс"
//        "Премиум"
let weight = +prompt('Введите вес посылки в кг', 0);
...
if (weight <= 0) {
  ...
  const basePrice = weight <= 1 ? 5 : weight <= 5 ? 10 : 15;
  const index = type === 'Стандарт' ? 1 : type === 'Экспресс' ? 1.5 :2;

  const totalPrice = basePrice * index;
